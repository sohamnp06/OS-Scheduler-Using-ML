...
def compute_features(processes):
    ...
    arrivals = np.array([p.arrival for p in processes])
    ...
    features = {
        ...
        "arrival_irregularity": float(np.var(arrivals)),
        "burst_time_skewness": safe_skew(burst_times),
        "arrival_range": float(np.max(arrivals) - np.min(arrivals)),
        "max_min_burst_ratio": float(np.max(burst_times) / np.min(burst_times)) if np.min(burst_times) > 0 else 1.0
    }

    return features

def get_queue_reason(features, algo):
    if features.get("burst_time_variance", 0) > 5 and algo in ["SJF", "SRTF"]:
        return f"Large burst-time variation. {algo} reduces waiting time for short processes."
    if features.get("priority_variance", 0) > 4 and algo == "PRIORITY":
        return f"High priority variance ({features.get('priority_variance'):.1f}). Priority scheduling prevents critical jobs from waiting too long."
    return f"ML Model selected {algo} based on optimal feature combination."
